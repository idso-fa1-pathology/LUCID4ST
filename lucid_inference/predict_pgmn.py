...
def generate_pgmn(datapath, save_dir, file_pattern='*.svs', nfile=0, patch_size=384, patch_stride=192, nClass=2, brown_black=False):
    ...
    for im_f in imgs:
        ...
        if not os.path.exists(os.path.join(save_dir_file, img_name + '.png')):
            ...
            if brown_black:
                #b2b option
                testImgc[:, :, 0] = testImgc[:, :, 2]
                testImgc[:, :, 1] = (testImgc[:, :, 1] * 0.9).astype(np.uint8)

            patch_obj = Patches(img_patch_h=patch_size, img_patch_w=patch_size, stride_h=patch_stride, stride_w=patch_stride, label_patch_h=patch_size, label_patch_w=patch_size)
