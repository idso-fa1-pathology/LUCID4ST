...
def ss1_stich(cws_folder, annotated_dir, output_dir, nfile=0, file_pattern='*.ndpi', downscale=0.0625):
    ...
    if os.path.exists(os.path.join(cws_folder, wsi_name, 'Ss1.jpg')):
        if os.path.exists(os.path.join(output_dir, wsi_name + "_Ss1.png")):
            ...
            w, h = int(slide_w * downscale), int(slide_h * downscale)
            ...
            for ii in images:
                ...
                w_i = int((cws_i - h_i_ori / cws_h * divisor_w)) * int(cws_w * downscale)
                h_i = int(np.floor(cws_i / divisor_w)) * int(cws_h * downscale)
                ...
                w_r = max(int(img.shape[1] * downscale), 1)
                h_r = max(int(img.shape[0] * downscale), 1)
